...
namespace cgl
{
	//Object that can load TGA or BMP files into memory and use as textures in openGL
	Image2D::Image2D()
	{
		for(int i = 0; i < 12; i++)
		{	
	...
	//Loads a BMP through auxDIBImageLoad(), glAUX, which is obsolete these days
	...
	//LoadBMP is prefered over LoadBMP2, loads a bmp image through reading it without calling another library's function. Returns true if success
	...
	//Loads a TGA image file, returns true if success
	...
	void Image2D::Draw(int positionX, int positionY, float width, float height, bool TRIANGLE_STRIP)
	{
		...
		if(TRIANGLE_STRIP)
		{
			glBegin(GL_QUADS);											// Begin Drawing A Single Quad
				glTexCoord2f(1.0f, 0.0f); glVertex2i(positionX+width,	positionY);
				glTexCoord2f(0.0f, 0.0f); glVertex2i(positionX,			positionY);
				glTexCoord2f(0.0f, 1.0f); glVertex2i(positionX,			positionY+height);
				glTexCoord2f(1.0f, 1.0f); glVertex2i(positionX+width,	positionY+height);
			glEnd();
		}
		else
		{
	...
	void Image2D::Draw(int positionX, int positionY, bool TRIANGLE_STRIP)
	{
		...
		if(TRIANGLE_STRIP)
		{
			glBegin(GL_TRIANGLE_STRIP);
				glTexCoord2f(1.0f, 0.0f); glVertex2i(positionX+this->width,	positionY);
				glTexCoord2f(0.0f, 0.0f); glVertex2i(positionX,				positionY);
				glTexCoord2f(0.0f, 1.0f); glVertex2i(positionX,				positionY+this->height);
				glTexCoord2f(1.0f, 1.0f); glVertex2i(positionX+this->width,	positionY+this->height);
		}
		else
		{
			glBegin(GL_QUADS);
