			...
			case TRI_VERTEXL:
				{
					...
					meshes[meshIndex].vertex = new Vertex_3DS[meshes[meshIndex].numVertex];
			...
			case TRI_UWMAP:
				{
					...
					meshes[meshIndex].texcoord = new TexCoord_3DS[meshes[meshIndex].numTexCoord];

					float *fpointer = (float*)(position+8);
					for(int i = 0; i < meshes[meshIndex].numTexCoord; i++)
						for(int ii = 0; ii < 2;ii++)
							meshes[meshIndex].texcoord[i][ii] = fpointer[(i*2)+ii];
			...
			case TRI_FACEL1:
				{
					...
					meshes[meshIndex].face = new Face_3DS[meshes[meshIndex].numFace];
					...
					meshes[meshIndex].normal = new Vertex_3DS[meshes[meshIndex].numFace];
...
void M_3DS::ConvertData()
{
	Vertex_3DS *vertexdata = new Vertex_3DS[totalFaces*3*2];		// Allocate vertex and normaldata in one go.
	Vertex_3DS *normaldata = &vertexdata[totalFaces*3];			// Set normaldata pointer
	...
	for(int i = 0; i < numMeshes; i++)
	{
		...
		{
			for(int iii = 0; iii < MAX_COORD; iii++)
			{
				// Alternative 1:
	...
	//glBufferData(GL_ARRAY_BUFFER, meshes[0].numVertex*sizeof(Vertex_3DS), meshes[0].vertex, GL_STATIC_DRAW);
	glBufferData(GL_ARRAY_BUFFER, totalFaces*sizeof(Vertex_3DS)*3, vertexdata, GL_STATIC_DRAW);
	glGenBuffers(1, &normalVBO);
	glBindBuffer(GL_ARRAY_BUFFER, normalVBO);
	glBufferData(GL_ARRAY_BUFFER, totalFaces*sizeof(Vertex_3DS)*3, normaldata, GL_STATIC_DRAW);
	...
	//glBufferData(GL_ELEMENT_ARRAY_BUFFER, meshes[0].numFace*sizeof(Face_3DS), meshes[0].face, GL_STATIC_DRAW);
...
void M_3DS::Draw()
{
	...
	glBindBuffer(GL_ARRAY_BUFFER,vertexVBO);
//	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexVBO);
	glVertexPointer(3, GL_FLOAT, 0/*sizeof(Vertex_3DS)*/, 0);
	glBindBuffer(GL_ARRAY_BUFFER,normalVBO);
